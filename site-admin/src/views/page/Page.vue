      ...
      <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
        <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
          <div class="relative overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
            <div v-if="selectedStorePage.length > 0"
            ...
            <table class="min-w-full divide-y divide-gray-300">
                ...
                <tr>
                  ...
                  <th scope="col" class="relative w-5 px-5 sm:w-5 sm:px-5">
                    <input type="checkbox"
                      ...
                      :checked="
                        indeterminate ||
                        selectedStorePage.length === sitePages.edges.length
                      " :indeterminate="indeterminate" @change="
                        selectedStorePage = $event.target.checked
                          ? sitePages.edges.map((p) => p.node.id)
                          : []
                      " />
                  </th>
              ...
              <tbody class="divide-y divide-gray-200 bg-white">
                <tr v-for="(item, index) in sitePages.edges" :key="item.node.id"
                  :class="[selectedStorePage.includes(item.node.id) && 'bg-gray-50']"
                  class="cursor-pointer active:cursor-wait">
                  <td class="whitespace-nowrap bg-yellow-100 text-sm text-gray-500 text-center">
                    {{ index + 1 }}
                  </td>
                  <td @click="selectStorePage(item.node.id, item.node.siteId)" class="relative w-5 px-5 sm:w-5 sm:px-5">
                    <div v-if="selectedStorePage.includes(item.node.id)"
                      class="absolute inset-y-0 left-0 w-0.5 bg-indigo-600"></div>
                    <input type="checkbox"
                      class="absolute left-3 top-1/2 -mt-2 h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 sm:left-3"
                      :value="item.node.id" v-model="selectedStorePage" />
                  </td>
                  <td @click="selectStorePage(item.node.id, item.node.siteId)"
              ...
              <div class="hidden sm:block">
                <p class="text-sm text-gray-700">
                  ...
                  <span class="font-medium">{{
                    sitePages.edges ? sitePages.edges.length : 0
                    }}</span>
                </p>
              </div>
...
const { getSitesByUser, setSiteId } = useSiteStore();
const first = ref(15);
...
// end parameter
const selectedStorePage = ref([]);
const checked = ref(false);
const indeterminate = computed(
  () =>
    selectedStorePage.value.length > 0 && selectedStorePage.value.length < sitePages.length
);
// query
...
const sitePages = computed(() => result.value?.sitePages ?? { edges: [] });
const loadMore = () => {
  fetchMore({
    variables: {
      siteId: selectedSite.value.id,
      after: sitePages.value.pageInfo.endCursor,
    },
...
// end query
const selectStorePage = (id, siteId) => {
  console.log(id, siteId)
  setPageId(id);
  setSiteId(siteId);
  router.replace(`/page/update/`);
};
