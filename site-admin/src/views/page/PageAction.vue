<template>
  <main :key="page.id" v-if="page" class="flex-1">
    <div class="relative mx-auto md:px-8 xl:px-0">
      <div class="flex items-center space-x-5">
        <div class="flex-shrink-0">
          <div class="relative">
            <img class="h-16 w-16 rounded-full" v-lazy="page.image" alt="" />
        ...
        <div>
          ...
          <!-- <p class="text-sm font-medium text-blue-700">
            <a :href="`https://${page.site.domain}/page-spotlight/${page.slug}/${page.hid}/`" target="_blank">{{ `/page-spotlight/${page.slug}/${page.hid}/` }}</a>
          </p> -->
        </div>
      </div>
      <div class="px-4 sm:px-6 md:px-0">
        <div class="py-0">
          ...
          <div v-if="selectedTab == 'Home' && page.schema" class="border border-t-0" :key="referenceId">
            <Editor :key="page.id + '_page' + referenceId" :page="'page'" :schema="page.schema"></Editor>
          </div>
...
import { SITE_PAGE_PREVIEW } from "@/gql/page";
...
const { pageId, page } = storeToRefs(usePageStore());
const { siteId } = storeToRefs(useSiteStore());
...
const { onResult, loading, error, refetch } = useQuery(SITE_PAGE_PREVIEW, {
  id: props.id ? props.id : pageId.value,
  siteId: props.siteId ? props.siteId : siteId.value,
});
