                    ...
                    <div class="relative overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
                      ...
                      <table class="min-w-full divide-y divide-gray-300">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                          <tr v-for="(item, index) in products.edges" :key="item.node.id"
                            class="cursor-pointer active:cursor-wait">
                            <td class="whitespace-nowrap bg-yellow-100 text-sm text-gray-500 text-center">
                              {{ index + 1 }}
                            </td>
                            <td @click="selectProduct(item.node)"
                              class="whitespace-nowrap w-full py-2 px-4 text-sm">
                              <div class="flex items-center">
                                <div class="h-10 w-10 flex-shrink-0">
                                  <img class="h-10 w-10 rounded-full" v-lazy="item.node.thumbnail" alt="" />
                                </div>
                                <div class="truncate max-w-[170px] ml-4">
                                  ...
                                  <div class="text-gray-500 text-xs">
                                    {{ formatMoney(item.node.price, item.node.currency) }}
                                  </div>
                                </div>
                              </div>
                            </td>
                            <td @click="selectProduct(item.node)"
                              class="whitespace-nowrap w-8 px-2 py-2 text-sm text-gray-500 font-semibold">
                              <div :class="item.node.quantity > 5
                                  ? 'text-green-400'
                                  : 'text-red-400'
                                ">
                                {{ item.node.quantity }}
