...
import {
  ...
  STORE_SUB_SUB_CATEGORY
} from "@/graphql/queries/catalog";
import { SELF_STORE_SUB_SUB_CATEGORY_UPDATE } from "@/graphql/mutations/catalog";
import { useAuthStore } from "@/store/useAuthStore";
import { useSiteStore } from "@/store/useSiteStore";
import SearchableMultiSelect from "@/components/SearchableMultiSelect";
import FilePondUploader from "@/components/FilePondUploader";

export default function SubSubCategoryEdit() {
  const { id } = useParams();
  const subSubCategoryId = Number(id);
  ...
  const { data } = useQuery(STORE_SUB_SUB_CATEGORY, {
    variables: { id: subSubCategoryId },
  ...
  const [updateSubSubCategory, { loading }] = useMutation(SELF_STORE_SUB_SUB_CATEGORY_UPDATE);

  useEffect(() => {
    const subSubCategory = data?.storeSubSubCategory;
    if (!subSubCategory) return;
    setCategoryId(subSubCategory.categoryId ? [Number(subSubCategory.categoryId)] : []);
    setSubCategoryId(subSubCategory.subCategoryId ? [Number(subSubCategory.subCategoryId)] : []);
    setTitle(subSubCategory.title ?? "");
    setTranslation(subSubCategory.translation ?? "");
    setDescription(subSubCategory.description ?? "");
    setPriority(Number(subSubCategory.priority ?? 0));
    setIsActive(Boolean(subSubCategory.isActive));
    setIsPrivate(Boolean(subSubCategory.isPrivate));
  }, [data]);
  ...
  const handleSave = async () => {
    if (!user?.id || !site?.id || !subSubCategoryId) return;
    await updateSubSubCategory({
      variables: {
        ...
        subCategoryId: subCategoryId.length ? subCategoryId[0] : undefined,
        title: title.trim(),
      ...
      <section className="rounded-2xl bg-white/90 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">Update nested category details.</p>
          </div>
          <button
            type="button"
