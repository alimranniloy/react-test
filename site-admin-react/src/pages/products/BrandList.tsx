...
import { STORE_BRANDS } from "@/graphql/queries/catalog";
...
export default function BrandList() {
  ...
  const { data, loading, refetch, fetchMore } = useQuery(STORE_BRANDS, {
    variables,
    skip: !siteId
  });

  const brands = data?.storeBrands?.edges?.map((edge: any) => edge.node) ?? [];
  const pageInfo = data?.storeBrands?.pageInfo;
  const totalAvailable = data?.storeBrands?.total ?? brands.length;
  const hasMore = (pageInfo?.hasNextPage ?? false) || brands.length < totalAvailable;
  ...
  const columns = useMemo<DataTableColumn<any>[]>(
    () => [
      ...
      },
      {
        ...
        cell: (brand) => (
          <Link to={`/product/brand/${brand.id}/edit`} className="text-indigo-600 hover:underline">
            Edit
          </Link>
        )
      }
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">Manage brand identities to keep your catalog consistent.</p>
          </div>
          <div>
            <Link
