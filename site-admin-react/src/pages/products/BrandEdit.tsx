...
import { STORE_BRAND } from "@/graphql/queries/brand";
import { SELF_STORE_BRAND_UPDATE } from "@/graphql/mutations/brand";
...
export default function BrandEdit() {
  ...
  const { data } = useQuery(STORE_BRAND, {
  ...
  const [updateBrand, { loading }] = useMutation(SELF_STORE_BRAND_UPDATE);

  useEffect(() => {
    const brand = data?.storeBrand;
    if (!brand) return;
    setTitle(brand.title ?? "");
    setTranslation(brand.translation ?? "");
    setDescription(brand.description ?? "");
    setPriority(Number(brand.priority ?? 0));
    setPriorityHome(Number(brand.priorityHome ?? 0));
    setIsActive(Boolean(brand.isActive));
    setIsHome(Boolean(brand.isHome));
    setIsPrivate(Boolean(brand.isPrivate));
    setSelectedCategories((brand.categories ?? []).map((value: number) => Number(value)));
    setSelectedSubCategories((brand.subCategories ?? []).map((value: number) => Number(value)));
  }, [data]);

  const handleSave = async () => {
    if (!user?.id || !site?.id || !brandId) return;
    await updateBrand({
      variables: {
        userId: user.id,
        siteId: site.id,
      ...
      <section className="rounded-2xl bg-white/90 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">Update brand logo, name, and visibility.</p>
          </div>
          <button
            type="button"
      ...
      <section className="rounded-2xl bg-white/90 p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <SearchableMultiSelect
            label="Categories"
            helperText="Show this brand under selected categories."
