...
export const AnimatedScreenSlider = (props: AnimatedScreenSliderProps) => {
  ...
  const callbacks = {
    ...
    goTo: (id: string) => {
      ...
      const existingIndex = paths.findIndex((path) => path === id);

      if (existingIndex !== -1) {
        setPaths((paths) => {
          return paths.slice(0, existingIndex + 1);
        });
        return;
      }
  ...
  React.useEffect(() => {
    ...
    if (currentPath === props.active) {
      return;
    }

    callbacks.goTo(props.active);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.active, currentPath]);

  React.useEffect(() => {
    if (!currentPath || !props.onScreenChange) {
      return;
    }

    props.onScreenChange(currentPath);
  }, [currentPath, props.onScreenChange]);
