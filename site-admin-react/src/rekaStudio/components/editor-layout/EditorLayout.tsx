...
import { ComponentOrderTool } from './ComponentOrderTool';
...
export const EditorLayout = observer(
  (
    ...
    const rightSidebarUiWidth = studioAI?.isOpen
      ? 440
    ...
    return (
      <React.Fragment>
        ...
        >
          <motion.div
            className={`overflow-x-visible overflow-y-auto relative h-full flex flex-col border-r border-solid border-outline bg-white ml-0 w-editor-left-sidebar`}
            initial={false}
            animate={[
              leftSidebarAnimate,
              ...
              <AnimatedScreenSlider
                ...
                onSetup={(getPath, goTo) => {
                  return autorun(() => {
                    ...
                    if (
                      selectedTpl &&
                      editor.leftSidebarScreen === 'component-list' &&
                      path === 'component-list'
                    ) {
                      editor.setLeftSidebarScreen('component-editor');
                    }

                    if (path === editor.leftSidebarScreen) {
                      return;
                    }

                    goTo(editor.leftSidebarScreen);
                  });
                }}
                screens={[
                  {
                      ...
                      return (
                        <ComponentList
                          onComponentSelected={(component) => {
                            editor.setLeftSidebarScreen('component-editor');
                            editor.router.push(
                              `/studio/?component=${encodeURIComponent(
                                component.name
                              )}`
                            );
                ...
                active={editor.leftSidebarScreen}
                onScreenChange={(screen) => {
                  if (screen === editor.leftSidebarScreen) {
                    return;
                  }

                  editor.setLeftSidebarScreen(screen as typeof editor.leftSidebarScreen);
                }}
                after={(name) => (
                  <React.Fragment>
                    <Carbonads
          ...
          <motion.div
            ...
            style={{
              width: `calc(100vw - ${
                ...
                Math.max(theme.width['editor-right-sidebar-code'], rightSidebarUiWidth)
              }px)`,
            }}
          >
            <ComponentEditorView />
          ...
          <motion.div
            className="relative h-full overflow-hidden border-l border-solid border-outline bg-white"
            initial={false}
            animate={{
              width:
          ...
          >
            ...
            <AnimatePresence initial={false}>
              {editor.mode === EditorMode.UI && (
                <motion.div
                  className="absolute top-0 right-0 flex h-full w-full flex-col overflow-auto bg-white"
                  initial="show"
                  animate="show"
                  exit="hide"
                  variants={{
