...
import {
  ensureUserFramesState,
  readUserFrames,
} from '@app/editor/userFrames';
...
export const AddFrameModal = (props: AddFrameModalProps) => {
  ...
  const existingFrame = props.frameId
    ? userFrames.find((frame) => frame.id === props.frameId)
    : null;
    ...
    >
      <div className="flex flex-col mt-5 gap-5">
        ...
        <Button
          ...
          onClick={() => {
            editor.reka.change(() => {
              const mutableUserFrames = ensureUserFramesState(editor.reka);

              if (!existingFrame) {
                mutableUserFrames.push({
                  id: frameName,
