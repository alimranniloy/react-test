                    ...
                    <TransitionChild as="template" enter="ease-out duration-300"
                        ...
                        <DialogPanel
                            ...
                            <div class="sm:flex sm:items-start">
                                <div
                                    class="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full sm:mx-0 sm:h-10 sm:w-10" :class="props.message.type == 'error' ? 'bg-red-100' : 'bg-green-100'">
                                    <ExclamationTriangleIcon v-if="props.message.type == 'error'" class="h-6 w-6 text-red-600" aria-hidden="true" />
                                    <CheckCircleIcon v-else class="h-6 w-6 text-green-600" aria-hidden="true" />
                                </div>
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    <DialogTitle as="h3" class="text-lg font-medium leading-6 text-gray-900">{{ props.message.title }}
                                    </DialogTitle>
                                    <div class="mt-2">
                                        <p class="text-sm text-gray-500">{{ props.message.message }}</p>
                                    </div>
                                </div>
                            </div>
