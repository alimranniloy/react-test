<template>
  <TransitionRoot appear :show="isOpen" as="template">
    <Dialog as="div" @close="closeModal" class="relative z-50">
      ...
      >
        <div
          class="max-w-xl bg-white w-full p-6 rounded-2xl hover-animation"
        >
          <TransitionChild
            as="template"
            enter="duration-300 ease-out"
            ...
            <DialogPanel class="flex flex-col items-center gap-6">
              ...
              >
                ...
                <div class="ml-auto flex items-center gap-3">
                  <button
                    @click="save()"
                    class="bg-red-400 rounded-md main-tab bg-accent-red py-1 px-4 font-bold text-white focus-visible:bg-accent-red/90 enabled:hover:bg-accent-red/90 enabled:active:bg-accent-red/80 disabled:brightness-75 "
                    type="button"
                  >
                    Save
                  </button>
              ...
              <section class="h-full overflow-y-auto scrollbar-thin transition-opacity w-full">
                <div class="group relative h-36 xs:h-44 sm:h-48">
                  ...
                  <img
                    v-lazy="me.cover"
                    class="h-full bg-light-line-reply w-full rounded-md"
                  />
                  <div
                  ...
                  >
                    <button
                      ...
                      class=" main-tab group/inner relative bg-accent-red/60 p-2 hover:bg-image-preview-hover/50 focus-visible:bg-image-preview-hover/50"
                      type="button"
                    >
                      <svg
                ...
                <div class="relative flex flex-col gap-6 py-3">
                  <div class="mb-8 xs:mb-12 sm:mb-14">
                    ...
                    />
                    <div
                      class="ml-4  group absolute aspect-square h-[100px] w-[100px] -translate-y-1/2 overflow-hidden rounded-full"
                    >
                      <figure
                      ...
                      >
                        <img
                          v-lazy="me.avatar"
                          alt="Bponi Dev"
                          class="rounded-full transition group-hover:brightness-75 duration-200 group-focus-within:brightness-75 object-cover h-[100px] w-[100px]"
                        />
                      </figure>
                      <button
                        @click="openAvatarPicker"
                        class=" main-tab group/inner absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-accent-red/60 p-2 hover:bg-image-preview-hover/50 focus-visible:bg-image-preview-hover/50"
                        type="button"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
...
import { SELF_USER_UPDATE } from "@/gql/me";
...
const handleAvatarUpload = async (event) => {
  ...
  if (file) {
    avatar.value = file;
    me.value.avatar = URL.createObjectURL(file);
  }
};
...
const handleCoverUpload = async (event) => {
  ...
  if (file) {
    cover.value = file;
    me.value.cover = URL.createObjectURL(file);
  }
};
const save = async () => {
  const { mutate, loading, error } = useMutation(SELF_USER_UPDATE, {
    variables: {
      userId: me.value.id,
      ...
      currency: me.value.currency,
      formattedAddress: me.value.formattedAddress,
      latitude: me.value.latitude,
      longitude: me.value.longitude,
      name: me.value.name,
      referedCode: me.value.referedCode,
    },
  });
  try {
    const response = await mutate();
    if (response.data.selfUserUpdate) {
      emit("refresh");
      addNotification(
