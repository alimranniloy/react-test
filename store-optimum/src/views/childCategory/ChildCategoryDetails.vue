        ...
        >
          ...
          <div>
            <h2 class="text-2xl font-bold">
              ...
              <span class="text-[--primary]"> {{ subCategory.title }}:</span>
            </h2>
            <div
              class="flex items-center flex-nowrap gap-5 whitespace-nowrap pb-3 mt-3 overflow-x-auto"
            >
              <button
                @click="router.go(-1), toggleSubSubCategory({ id: null })"
              ...
              <button
                @click="toggleSubSubCategory({ id: null })"
                class="px-3 py-2 rounded-2xl shadow transition-all mb-1"
                :class="subSubCategoryId == null ? 'bg-green-100' : 'bg-white'"
              >
                All
              ...
              <button
                ...
                v-for="item in subSubCategories.filter(
                  (v) => v.subCategoryId == subCategory.id
                )"
                class="px-3 py-2 rounded-2xl shadow transition-all mb-1"
                :class="
                  subSubCategoryId == item.id ? 'bg-green-100' : 'bg-white'
                "
              >
                {{ item.title }}
              </button>
          ...
          >
            <div class="text-xl font-medium">{{ subCategory.title }}</div>
          ...
          <div v-html="subCategory.description" class="pt-10 px-4"></div>
        </div>
      </div>
    </div>
  </main>
...
const pages = [{ name: "Child Category", href: "#", current: false }];
...
const toggleSubCategory = (data) => {
  if (data.id == null) {
    ...
    router.push(`/sub-category/${data.title}/${data.hid}`);
    subSubCategoryId.value = null;
  }
};
const toggleSubSubCategory = (data) => {
  if (data.id == null) {
    ...
    subSubCategoryId.value = data.id;
  }
};
</script>
