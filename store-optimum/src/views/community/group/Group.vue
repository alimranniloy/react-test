    ...
    <main class="hover-animation flex min-h-screen w-full max-w-xl flex-col pb-96">
        <header v-if="!isIframe"
            ...
            <a @click="$router.back()"
                ...
                <ArrowLeftIcon class="h-5 w-5 dark:text-gray-200"></ArrowLeftIcon>
                <div
        ...
        <section v-if="member == null" data-projection-id="190" style="opacity: 1"
            class="dark:bg-gray-900 rounded-t-lg">
                    ...
                    <div class="flex flex-wrap gap-x-3 gap-y-1 text-light-secondary">
                        ...
                        <div class="flex items-center gap-1">
                            <i>
                                ...
                                }`" target="_blank" rel="noreferrer">{{ community ? community.title : "guest"
                                }}</router-link>
                        </div>
        ...
        >
          <router-link
            ...
            :to="`/group/${community ? community.slug : 'guest'}/${
              community ? community.hid : '6'
            }/members/`"
        ...
        <nav class="hover-animation flex justify-between overflow-y-auto scrollbar-thin border-b border-light-border dark:border-dark-border dark:bg-gray-800 rounded-b-lg dark:mb-4"
        ...
        <Create v-if="me" :community="community" class="md:block" />
        <FeedList v-if="community" :key="community.id" :communityId="community.id" />
...
const { setTempCommunity } = useCommunityStore();
...
if (
    ...
    router.currentRoute.value.name == "CommunityGroup"
) {
...
const getMember = async () => {
    if (me.value && community.value) {
        const { onResult, refetch, onError } = useQuery(FEED_MEMBER_FIND_BY_USER, {
            userId: me.value.id,
            communityId: community.value.id,
        });
        onResult((queryResult) => {
            setTempCommunity(community.value);
            console.log(queryResult);
            member.value = queryResult.data.feedMemberFindByUser;
        });
        onError((error) => {
            member.value = null;
        });
