...
import { STORE_TAGS } from "@/graphql/queries/catalog";
...
export default function TagList() {
  ...
  const { data, loading, refetch, fetchMore } = useQuery(STORE_TAGS, {
    variables,
    skip: !siteId
  });

  const tags = data?.storeTags?.edges?.map((edge: any) => edge.node) ?? [];
  const pageInfo = data?.storeTags?.pageInfo;
  const totalAvailable = data?.storeTags?.total ?? tags.length;
  const hasMore = (pageInfo?.hasNextPage ?? false) || tags.length < totalAvailable;
  ...
  const columns = useMemo<DataTableColumn<any>[]>(
    () => [
      ...
      },
      {
        ...
        cell: (tag) => (
          <Link to={`/product/tag/${tag.id}/edit`} className="text-indigo-600 hover:underline">
            Edit
          </Link>
        )
      }
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-semibold text-slate-900">Tag</h2>
              <button
                type="button"
                onClick={() => refetch()}
            ...
            <p className="text-sm text-slate-500">Create tags to power advanced filtering and merchandising.</p>
          </div>
          <div>
            <Link
