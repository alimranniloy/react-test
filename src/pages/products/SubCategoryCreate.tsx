...
import { SELF_STORE_SUB_CATEGORY_CREATE } from "@/graphql/mutations/catalog";
...
export default function SubCategoryCreate() {
  ...
  const [createSubCategory, { loading }] = useMutation(SELF_STORE_SUB_CATEGORY_CREATE);

  const handleSave = async () => {
    if (!user?.id || !site?.id || !title.trim() || !imageFile || !categoryId.length) return;
      ...
      <section className="rounded-2xl bg-white/90 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">Add a new sub category under a parent category.</p>
          </div>
          <button
            type="button"
