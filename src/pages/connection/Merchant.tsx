...
import { STORE_SHOPS } from "@/graphql/queries/connection";
...
export default function Merchant() {
  ...
  const { data, loading, refetch, fetchMore } = useQuery(STORE_SHOPS, {
    variables,
    skip: !siteId
  });

  const rows = data?.storeShops?.edges?.map((edge: any) => edge.node) ?? [];
  const pageInfo = data?.storeShops?.pageInfo;
  const totalAvailable = data?.storeShops?.total ?? rows.length;
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <p className="mt-2 text-sm text-gray-700">Manage merchant shops and their performance.</p>
      </div>
