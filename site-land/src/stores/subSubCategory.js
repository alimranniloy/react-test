...
export const useSubSubCategoryStore = defineStore("subSubCategory_" + import.meta.env.VITE_VERSION, {
  state: () => ({
  ...
  actions: {
    ...
    async getSubSubCategories(siteId, childId) {
      const { onResult, loading, error } = useQuery(STORE_SUB_SUB_CATEGORIES, {
        siteId: siteId,
        childId: childId,
        first: 2048,
      });
